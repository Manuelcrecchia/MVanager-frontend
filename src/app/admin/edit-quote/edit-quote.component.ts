...
export class EditQuoteComponent {
  ...
  ngOnInit() {
    const numeroPreventivo =
      this.route.snapshot.paramMap.get('numeroPreventivo') ||
    ...
    this.globalService.loadTenantConfig(false, { showError: false }).then(() => {
      ...
      if (numeroPreventivo) {
        this.caricaPreventivoFromDb(numeroPreventivo);
      }
    });
  }

  private caricaPreventivoFromDb(numeroPreventivo: string): void {
    this.http
      .post(
